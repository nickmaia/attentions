...
# Attention Bahdanau 
# score: v_a(tanh(W_q * q + W_k * k))
class AttentionBahdanau(nn.Module):
    def __init__(self, d_model, head_dim, num_heads, attention_dim=None, mask=False, dropout=0.1):
        super().__init__()
        ...
        self.mask = mask
        self.attention_dim = attention_dim or head_dim
        ...
        self.W_q = nn.Linear(head_dim, self.attention_dim, bias=False)
        self.W_k = nn.Linear(head_dim, self.attention_dim, bias=False)
    ...
    def forward(self, x_q, x_kv=None, key_padding_mask=None):
        ...
        attn_scores = q_proj.unsqueeze(3) + k_proj.unsqueeze(2)
        attn_scores = self.v_a(torch.tanh(attn_scores)).squeeze(-1)
